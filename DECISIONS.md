...
| Pocket TTS 2.1 Peter Yearsley preset | **Voice passes; raw numeric path failed; blind A/B pending** | Dave judged the voice decent/good, but the raw-input audition handled numbers/currency poorly. A validated raw-versus-normalized pair is ready. Long-form completeness and cloning remain unproven. |
| NeuTTS Air 1.4.1 Q4 + Jo | **Voice passes; raw numeric path failed; blind A/B pending** | Dave judged the voice decent/good, but the raw-input audition handled numbers/currency poorly. A validated sentence-chunked raw-versus-normalized pair is ready; chunking remains mandatory. |
| KittenTTS 0.8.1 Jasper/Rosie | **Both voices pass; raw numeric path failed; blind A/B pending** | Dave judged both preset voices decent/good, but the raw-input auditions handled numbers/currency poorly. Validated raw-versus-normalized pairs are ready; these remain preset-only with no UK-identity claim. |
...
follow-up holds model, voice and settings fixed and changes only raw versus
explicit spoken wording. All eight arms are complete and ready for blind
listening. Do not assign the fault to either engine or preprocessing until Dave
grades those pairs. None of these candidates replaces the Chatterbox
Nano/Beatrice production default yet.
