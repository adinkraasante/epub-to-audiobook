...
| EdgeTTS accented English voices (2026-08-14) | The only currently heard regional-accent option that comes close. All tested Chinese company names were still pronounced badly, so it is not approved for Chinese-business nonfiction without a supported pronunciation fix. A same-William/same-text/same-format Edge/Azure endpoint control is structurally verified and awaiting the listening verdict. | Microsoft cloud service via unofficial pinned `edge-tts==7.2.8` interface |
| Azure Speech Standard native AU/IE/ZA voices (2026-08-15) | **Accent pass / overall rejection:** Darren, Connor and Luke had spot-on Australian, Irish and South African accents, but all sounded robotic and degraded. The exact 24 kHz/160 kbps MP3 path is not production-approved. The earlier Edge comparison used a different speaker; one exact William endpoint control is now rendered at matched 24 kHz/48 kbps and awaits listening. Microsoft also documents a separate high-fidelity 48 kHz standard model; prosody cannot replace the voices' core timbre. | Azure Speech F0, UK South |
