...
| Gemini 3.1 Flash TTS / Achernar | Free-only adapter live; exact preview passed; 10:10 technical gate ready for listening | GBP0 on an unbilled Developer API Free project | Short Studio/app preview was very good. Five one-attempt paragraph packs produced a complete, fully decoded 1,644-word gate after one zero-output 503 and manual resume. Current official SDK/API only, resumable cache, no paid/Vertex/Batch fallback. Free content may train Google products; ten requests/day makes whole books slow. |
...
| [IndexTTS-2.5](https://github.com/index-tts/index-tts/blob/39207d91c30899cad1e7c1b9eb678c241f678e55/README.md) | Stable `v2.5.0` shipped 2026-08-13 with voice cloning, token-aware 120-token splitting, 200 ms joins, `duration_factor` pace control and CMU English pronunciation annotations. The Bilibili Model Use License is not permissive OSS. | First run an FP32 load + 30-second capacity gate on a free Kaggle T4, pinned to model snapshot `c39ce5ba981572cb187443877ff559dfb246ce63`; never P100 or paid compute. If it fits and Dave accepts the short normalized arm, render the 8–10-minute Arthur gate. |
