...
def instrument_html(html_content):
    ...
    for tag in soup.find_all(['p', 'div', 'li', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6']):
        if not tag.string and len(tag.contents) == 0:
            continue
        
        # Simple sentence tokenization on the tag's text
        text = tag.get_text()
...
def generate_smil(html_file_name, audio_file_name, span_count, duration_per_span):
    ...
    # This is a highly simplified mapping: divide total duration equally among spans
    # Real alignment would use the actual duration_s from chunks.jsonl
    smil = [
    ...
    try:
        ...
        for ch_idx, mp3_path in audio_files.items():
            audio_item = epub.EpubItem(
                uid=f"audio_{ch_idx}",
                file_name=f"Audio/{mp3_path.name}",
                media_type="audio/mpeg",
                content=mp3_path.read_bytes()
            )
            book.add_item(audio_item)
            audio_items[ch_idx] = audio_item

        # Find HTML items in spine
        spine_items = [book.get_item_with_id(iid) for iid, _ in book.spine if iid != 'nav']
        ...
        for idx, html_item in enumerate(html_items):
            ...
            if ch_idx in audio_items:
                ...
                # 1. Instrument HTML with spans
                new_html, span_count = instrument_html(html_item.content)
                html_item.content = new_html.encode('utf-8')
                
                # 2. Estimate duration (Fallback logic for MVP)
                # Ideally we'd use duration_s from chunks.jsonl mapped to this chapter
                # For now, let's just get the actual MP3 length
                try:
                    from mutagen.mp3 import MP3
                    audio_info = MP3(audio_files[ch_idx])
    ...
    try:
        ...
        if opf_path:
            ...
            for idx, html in enumerate(html_items):
                ch_idx = idx + 1
                if any(s.uid == f"smil_{ch_idx}" for s in smil_items):
                    # Inject media-overlay into manifest item
                    content = content.replace(f'id="{html.uid}"', f'id="{html.uid}" media-overlay="smil_{ch_idx}"')
