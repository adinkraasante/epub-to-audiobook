...
VOICES = {
    ...
    # --- The same accents on NANO, cloned from NATIVE-MODEL output ---
    # REPLACED the twelve `vctk_*` Chatterbox clones on 2026-07-27. Dave graded
    # those "utter shit" and they are gone, WAVs included — there is no value in
    # offering a choice between a good voice and a known-bad one.
    #
    # The reference audio is the difference. The old clones were conditioned on
    # raw VCTK: anechoic, close-mic, short disconnected sentences. These are
    # conditioned on continuous prose generated by the native-accent Piper
    # model — the voices Dave just approved — which is both smoother and
    # actually carries the accent it is supposed to.
    #
    # WAVs live in CUSTOM_VOICES_DIR because that is the only path into the
    # chatterbox container without an image rebuild; the entries here give them
    # real names instead of "your clone".
    'accent_irish_male_nano': {'name': 'Irish male — Donegal (Nano)', 'accent': 'Irish', 'gender': 'Male', 'engine': 'chatterbox_nano'},
    'accent_irish_female_nano': {'name': 'Irish female — Dublin (Nano)', 'accent': 'Irish', 'gender': 'Female', 'engine': 'chatterbox_nano'},
    'accent_northernirish_male_nano': {'name': 'Northern Irish male — Belfast (Nano)', 'accent': 'Northern Irish', 'gender': 'Male', 'engine': 'chatterbox_nano'},
    'accent_northernirish_female_nano': {'name': 'Northern Irish female — Belfast (Nano)', 'accent': 'Northern Irish', 'gender': 'Female', 'engine': 'chatterbox_nano'},
    'accent_australian_male_nano': {'name': 'Australian male — Sydney (Nano)', 'accent': 'Australian', 'gender': 'Male', 'engine': 'chatterbox_nano'},
    'accent_australian_female_nano': {'name': 'Australian female (Nano)', 'accent': 'Australian', 'gender': 'Female', 'engine': 'chatterbox_nano'},
    'accent_welsh_female_nano': {'name': 'Welsh female — Cardiff (Nano)', 'accent': 'Welsh', 'gender': 'Female', 'engine': 'chatterbox_nano'},
    'accent_scottish_male_nano': {'name': 'Scottish male — Edinburgh (Nano)', 'accent': 'Scottish', 'gender': 'Male', 'engine': 'chatterbox_nano'},
    'accent_scottish_female_nano': {'name': 'Scottish female — Edinburgh (Nano)', 'accent': 'Scottish', 'gender': 'Female', 'engine': 'chatterbox_nano'},
...
def audition_sample(name: str):
    ...
    if name not in ('ab_1997_raw', 'ab_1997_spelled',
       ...
       and not (name.startswith(('ac_', 'na_')) and re.fullmatch(r'[a-z0-9_]+', name)):
